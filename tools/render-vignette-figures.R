...
post_bsvar <- estimate(spec_bsvar, S = 100, thin = 1, show_progress = FALSE)
spec_bsvar_alt <- specify_bsvar$new(us_fiscal_lsuw, p = 3)
post_bsvar_alt <- estimate(spec_bsvar_alt, S = 100, thin = 1, show_progress = FALSE)

cdm_bsvar <- cdm(post_bsvar, horizon = 8)
.save_base(
  "vignettes/figures/cdm-showcase.png",
  plot(cdm_bsvar)
)

cmp_irf <- compare_irf(p1 = post_bsvar, p2 = post_bsvar_alt, horizon = 8)
...
rep_bsvar <- median_target_irf(post_bsvar, horizon = 8)
.save_gg(
...
post_sign <- estimate(spec_sign, S = 100, thin = 1, show_progress = FALSE)
