...
`bsvarPost` should stay a post-estimation companion package for `bsvars`
and `bsvarSIGNs`.
...
- [`cdm()`](https://davidzenz.github.io/bsvarPost/reference/cdm.md) for
  posterior objects from `bsvars` and `bsvarSIGNs`
...
- bridge helpers:
  - [`as_apr_cond_forc()`](https://davidzenz.github.io/bsvarPost/reference/as_apr_cond_forc.md)
  - [`tidy_apr_forecast()`](https://davidzenz.github.io/bsvarPost/reference/tidy_apr_forecast.md)
  - [`apr_gen_mats()`](https://davidzenz.github.io/bsvarPost/reference/apr_gen_mats.md)
- optional `tsibble` conversion via
  [`as_tsibble_post()`](https://davidzenz.github.io/bsvarPost/reference/as_tsibble_post.md)
...
- choose a single posterior draw / structural model closest to posterior
  median IRFs or CDMs
...
Posterior evaluation of sign, zero, ranking, and narrative-style
restrictions.
...
- make restrictions inspectable as posterior objects rather than hidden
  inside estimation choices
...
- magnitude restrictions are treated as post-estimation audits in
  `bsvarPost`
...
- make the historical-decomposition side of the package as usable as
  IRF/CDM workflows
...
- remove the need for users to hand-code common paper tables from
  IRFs/CDMs
...
- one introductory vignette focused on the main `bsvars` and
  `bsvarSIGNs` workflows
- one applied-workflows vignette covering representative summaries,
  audits, historical decomposition events, and response-timing summaries
...
- make the package read like a standard R community package rather than
  a developer notebook
- ensure the vignettes are extensive enough to serve as the main user
  guide
...
- `pkgdown` solves vignette discoverability, cross-references, and
  navigation much more cleanly
- the package now has enough surface area that a website is more usable
  than relying on
  [`browseVignettes()`](https://rdrr.io/r/utils/browseVignettes.html)
  and package help pages alone
...
- homepage that positions `bsvarPost` as a post-estimation companion
  package
- working cross-links between reference topics and vignettes

Goal:

- make the package documentation navigable in the standard modern
  R-package way
- provide reliable direct access to vignettes and cross-referenced help
  topics
...
Every feature tranche should be treated as incomplete until package
checks are green both locally and in GitHub Actions.
...
- keep workflow configuration and package metadata aligned with the
  actual dependency and vignette surface
...
publication-ready figures that match journal, institution, or
project-specific styles.

Planned capabilities:

- themeable plotting wrappers for `bsvars`, `bsvarSIGNs`, and
  `bsvarPost` outputs
...
- make `bsvarPost` useful not just for computation but also for final
  research output
...
3.  [`restriction_audit()`](https://davidzenz.github.io/bsvarPost/reference/restriction_audit.md)
4.  [`magnitude_audit()`](https://davidzenz.github.io/bsvarPost/reference/magnitude_audit.md)
5.  [`tidy_hd_event()`](https://davidzenz.github.io/bsvarPost/reference/tidy_hd_event.md)
6.  [`shock_ranking()`](https://davidzenz.github.io/bsvarPost/reference/shock_ranking.md)
7.  [`peak_response()`](https://davidzenz.github.io/bsvarPost/reference/peak_response.md)
    /
    [`duration_response()`](https://davidzenz.github.io/bsvarPost/reference/duration_response.md)
8.  [`compare_restrictions()`](https://davidzenz.github.io/bsvarPost/reference/compare_restrictions.md)
9.  [`acceptance_diagnostics()`](https://davidzenz.github.io/bsvarPost/reference/acceptance_diagnostics.md)
...
Magnitude restrictions are worth supporting in `bsvarPost`, but only in
the following sense:
...
They should not be implemented as estimation-time admissibility filters
in this package.
